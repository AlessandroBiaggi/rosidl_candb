...
def generate(
    ...
    template_ctx = TemplateContext(
        ...
        globals={
            ...
            'database_name': database_name,
            'node_names': node_names or [n.name for n in db.nodes],
            'messages': messages,
    ...
    pack_header_output_file = output_dir / naming_convention.pack_header_path()
    pack_header_output_file = pack_header_output_file.resolve().absolute()

    try:
        template_ctx.expand_template(
            template_name='db__pack.hpp.em',
            output_file=pack_header_output_file,
        )
    except Exception as e:
        raise RuntimeError(f"Error generating header file '{pack_header_output_file}'") from e

    output_files.append(pack_header_output_file)

    unpack_header_output_file = output_dir / naming_convention.unpack_header_path()
    unpack_header_output_file = unpack_header_output_file.resolve().absolute()

    try:
        template_ctx.expand_template(
            template_name='db__unpack.hpp.em',
            output_file=unpack_header_output_file,
        )
    except Exception as e:
        raise RuntimeError(f"Error generating header file '{unpack_header_output_file}'") from e

    output_files.append(unpack_header_output_file)

    for message in messages:
        pack_header_output_file = output_dir / naming_convention.message_pack_header_path(message.name)
        pack_header_output_file = pack_header_output_file.resolve().absolute()

        try:
            template_ctx.expand_template(
                template_name='msg__pack.hpp.em',
                output_file=pack_header_output_file,
                locals={'message': message},
            )
        except Exception as e:
            raise RuntimeError(f"Error generating source file '{pack_header_output_file}'") from e

        output_files.append(pack_header_output_file)

        unpack_header_output_file = output_dir / naming_convention.message_unpack_header_path(message.name)
        unpack_header_output_file = unpack_header_output_file.resolve().absolute()

        try:
            template_ctx.expand_template(
                template_name='msg__unpack.hpp.em',
                output_file=unpack_header_output_file,
                locals={'message': message},
            )
        except Exception as e:
            raise RuntimeError(f"Error generating source file '{unpack_header_output_file}'") from e
