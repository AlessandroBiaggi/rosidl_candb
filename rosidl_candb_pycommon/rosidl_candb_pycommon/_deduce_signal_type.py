...
def deduce_signal_type(
    ...
    if raw:
        ...
        num_val_bits = signal.length - 1 if signal.is_signed else signal.length
        ...
        if _is_integer(signal.scale) and _is_integer(signal.offset):
            ...
            if signal.is_signed not in types:
                raise RuntimeError(f"Unsupported {'signed' if signal.is_signed else 'unsigned'} int signals")
            types = types[signal.is_signed]
