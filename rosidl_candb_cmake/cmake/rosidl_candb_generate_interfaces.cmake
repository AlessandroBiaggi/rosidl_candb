#
# Generate code for ROS MSG files using all available generators.
#
...
#   generation step
# :type target: string
# :param ARGN: the interface file containing message
...
#   rosidl_candb_interface_packages group
...
macro(rosidl_candb_generate_interfaces target)
    ...
    set(_target_dependencies
            "${rosidl_candb_adapter_TEMPLATE_DIR}/message.j2"
            "${rosidl_candb_adapter_TEMPLATE_DIR}/message__comment.j2"
            "${rosidl_candb_adapter_TEMPLATE_DIR}/message__constants.j2"
            "${rosidl_candb_adapter_TEMPLATE_DIR}/message_stamped.j2"
    )
    ...
    if (NOT _ARG_SKIP_INSTALL)
        ...
        foreach (_msg_tuple ${_msg_tuples})
            string(REGEX REPLACE ":([^:]*)$" ";\\1" _msg_list "${_msg_tuple}")

            list(GET _msg_list 1 _msg_relpath)
            file(TO_CMAKE_PATH "${_msg_relpath}" _msg_relpath)

            list(APPEND _msg_file_lines "${_msg_relpath}")
        endforeach ()

        list(SORT _msg_files_lines)
        string(REPLACE ";" "\n" _msg_files_lines "${_msg_files_lines}")

        ament_index_register_resource("rosidl_candb_interfaces" CONTENT "${_msg_files_lines}")
    endif ()
